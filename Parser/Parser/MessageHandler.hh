...
*/
enum MESSAGE_LEVEL {LOG=0, DEBUG=1, INFO=2, WARNING=3, ERROR=4,
		    CRITICAL=5, EXCEPTION=6};
...
public:
  ...
  template<class MsgAction> int AddMessenger(MESSAGE_LEVEL thresh,MsgAction act)
  { 
    _messengers.push_back(new Messenger<MsgAction>(thresh,act)); 
    return _messengers.size()-1; 
  } 
  /// Remove an already registered messenger
  void RemoveMessenger(size_t index=0);
  /// Set the severity threshold for which messages to print
  void SetThreshold(MESSAGE_LEVEL thresh, int index=-1);
...
private:
  ...
  MessageHandler(const MessageHandler& right) {}
  MessageHandler& operator=(const MessageHandler& right) {return *this;}
  ...
  std::vector<VMessenger*> _messengers;
