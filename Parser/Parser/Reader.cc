...
RawEventPtr Reader::GetEventWithID(uint32_t id)
{
  ...
  while( _ghead.global_header_version > 0 && id > _ghead.event_id_max ){
    if(OpenNextFile()){
...
int Reader::OpenNextFile()
{
  ...
  if(_current_file_index == _unset_file_index){
    ...
    _fin = gzopen(_filename.c_str(),"rb");
  }
  if(!_fin){
    ...
    while( (pos = filename.rfind('.')) != std::string::npos)
      filename.resize(pos);
    std::stringstream fname;
    fname<<filename<<"."<<std::setw(3)<<std::setfill('0')
     <<++_current_file_index<<".out";
    Message(DEBUG)<<"Attemptng to open split file "<<fname.str()<<std::endl;
    _fin = gzopen(fname.str().c_str(), "rb");
  }
  ...
  if(_ghead.magic_num_check != magic_number){
    ...
    Message(DEBUG)<<"File header information:"
